...
describe("shouldOpenNewTab", () => {
  ...
  it("rejects Cmd+Shift+T", () => {
    expect(
      shouldOpenNewTab({ ...down, key: "t", metaKey: true, shiftKey: true }, true),
    ).toBe(false);
  });
  it("rejects on keyup", () => {
...
describe("shouldCloseCurrentTab", () => {
  ...
  it("rejects Cmd+Shift+W", () => {
    expect(
      shouldCloseCurrentTab(
        { ...down, key: "w", metaKey: true, shiftKey: true },
        true,
      ),
    ).toBe(false);
  });
...
describe("resolveTabIndexShortcut", () => {
  it("maps Cmd+1..9 to zero-based index 0..8 on mac", () => {
    for (let i = 1; i <= 9; i += 1) {
      expect(
        resolveTabIndexShortcut(
          { ...down, key: String(i), metaKey: true },
          true,
        ),
      ).toBe(i - 1);
    }
  ...
  it("maps Ctrl+1..9 on linux", () => {
    expect(
      resolveTabIndexShortcut({ ...down, key: "3", ctrlKey: true }, false),
    ).toBe(2);
  });
  it("returns null for Cmd+0 (intentionally unbound)", () => {
    expect(
      resolveTabIndexShortcut({ ...down, key: "0", metaKey: true }, true),
    ).toBeNull();
  });
  ...
  it("returns null when Shift / Alt are pressed", () => {
    expect(
      resolveTabIndexShortcut(
        { ...down, key: "1", metaKey: true, shiftKey: true },
        true,
      ),
    ).toBeNull();
    expect(
      resolveTabIndexShortcut(
        { ...down, key: "1", metaKey: true, altKey: true },
        true,
      ),
    ).toBeNull();
  });
  it("returns null on keyup", () => {
    expect(
      resolveTabIndexShortcut({ type: "keyup", key: "1", metaKey: true }, true),
    ).toBeNull();
  });
});
