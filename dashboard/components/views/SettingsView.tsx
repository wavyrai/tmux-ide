...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (value: boolean) => void }) {
  return (
    <button
      ...
      className={`h-5 w-9 border transition-colors ${
        checked
    ...
    >
      <span
        className={`block h-4 w-4 bg-[var(--bg)] transition-transform ${
...
export function SettingsView() {
  ...
  return (
    <div data-testid="settings-view" className="flex h-full flex-1 bg-[var(--bg)]">
      <nav className="w-52 shrink-0 border-r border-[var(--border-weak)] bg-[var(--bg-strong)] p-2">
        {sections.map((section) => (
          <button
            ...
            className={`block h-8 w-full px-2 text-left text-[12px] transition-colors ${
              active === section.id
      ...
      <main className="min-w-0 flex-1 overflow-auto p-5">
          ...
          <section data-testid="settings-section-general" className="max-w-3xl">
            ...
            >
              <select
                ...
                onChange={(event) =>
                  settings.setGeneral({
                    defaultActivity: event.target.value as typeof settings.general.defaultActivity,
                  })
                }
            ...
            <SettingRow title="Default project tab" description="Initial project view preference.">
              <select
        ...
        {active === "appearance" && (
          <section data-testid="settings-section-appearance" className="max-w-4xl">
            <h2 className="mb-3 text-[15px] text-[var(--accent)]">Appearance</h2>
        ...
        {active === "keybinds" && (
          <section data-testid="settings-section-keybinds" className="max-w-4xl">
            <h2 className="mb-3 text-[15px] text-[var(--accent)]">Keybinds</h2>
            {conflict && <div className="mb-2 text-[11px] text-[var(--yellow)]">{conflict}</div>}
            <div className="border border-[var(--border-weak)]">
              {actions.map((action) => {
                const effective = settings.keybinds[action.id] ?? action.keybind ?? "";
                return (
                  <div
                    key={action.id}
                    className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 border-b border-[var(--border-weak)] px-3 py-2 last:border-b-0"
                  >
                    <div className="min-w-0">
                    ...
                    <kbd
                      ...
                      className="min-w-24 border border-[var(--border)] bg-[var(--surface)] px-2 py-1 text-center text-[11px] text-[var(--fg-secondary)]"
                    >
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="border border-[var(--border)] px-2 py-1 text-[11px] text-[var(--fg-secondary)] hover:border-[var(--accent)] hover:text-[var(--accent)]"
                      >
                        edit
                      </button>
                      <button
                        ...
                        className="border border-[var(--border)] px-2 py-1 text-[11px] text-[var(--dim)] hover:text-[var(--fg)]"
                      >
                        reset
                      </button>
                    </div>
        ...
        {active === "terminal" && (
          <section data-testid="settings-section-terminal" className="max-w-3xl">
            <h2 className="mb-3 text-[15px] text-[var(--accent)]">Terminal</h2>
            <SettingRow title="Font size" description="Applies to existing terminals.">
              ...
              />
              <span className="w-10 text-right text-[12px] text-[var(--dim)]">
            ...
            <SettingRow title="Scrollback" description="Number of terminal lines retained.">
              <input
                ...
                className="w-28 border border-[var(--border)] bg-[var(--surface)] px-2 py-1 text-[12px]"
              />
            </SettingRow>
        ...
        {active === "sounds" && (
          <section data-testid="settings-section-sounds" className="max-w-3xl">
            <h2 className="mb-3 text-[15px] text-[var(--accent)]">Sounds</h2>
            {[
        ...
        {active === "about" && (
          <section data-testid="settings-section-about" className="max-w-3xl">
            <h2 className="mb-3 text-[15px] text-[var(--accent)]">About</h2>
