...
export function TaskRow({ task: t, selected = false, onSelect }: TaskRowProps) {
  ...
  return (
    <div>
      <div
        className={`flex items-center h-6 px-2 cursor-pointer ${bg}`}
        onClick={onSelect}
      >
        ...
        {t.assignee && (
          <span className="shrink-0 pl-2 text-[var(--cyan)]">@{t.assignee}</span>
        )}
      </div>
      {selected && (
        <div className="pl-[14ch] py-1 space-y-0.5 text-[var(--dim)]">
          {t.description && (
            <div className="text-[var(--fg)]">{t.description}</div>
          )}
