...
struct GhosttyTerminalView: NSViewRepresentable {
    ...
    final class Coordinator {
        ...
        func installPortal(runtimeView: NSView, placeholder: NSView) {
            ...
            // Without this, animations (Claude spinner, Gemini CLI) freeze
            // because the surface was marked occluded when removed.
        ...
        func removePortal(runtimeView: NSView?) {
            ...
            // (e.g. during a split-pane tree restructure where dismantleNSView fires
            // after the replacement view's installPortal).
    ...
    func updateNSView(_ nsView: PortalPlaceholderView, context: Context) {
        ...
        nsView.onLayout = { [weak runtimeView, weak nsView] in
            guard let runtimeView, let nsView else { return }
            coordinator.updatePortalFrame(runtimeView: runtimeView, placeholder: nsView)
            // Also trigger surface resize
            if let surface = (runtimeView as? GhosttyNativeView)?.terminalSurface {
                surface.resizeToCurrentViewBounds()
            }
        }

        nsView.onWindowChanged = { [weak surface, weak runtimeView, weak nsView] in
            guard let surface, let runtimeView, let nsView else { return }
