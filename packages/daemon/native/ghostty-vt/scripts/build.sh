...
if [[ -z "$node_include" ]]; then
  for candidate in /usr/include/node /usr/local/include/node /opt/homebrew/include/node; do
...
"$script_dir/verify-host-tools.sh"

case "$(uname -s)" in
  Darwin)
    SOURCE_DATE_EPOCH=0 cc -std=c11 -O3 -Wall -Wextra -Werror \
    ...
    strip -S "$stage/ghostty_vt_proof.node"
    ;;
  Linux)
    strip --strip-all "$stage/install/lib/libghostty-vt.so.0.1.0"
    SOURCE_DATE_EPOCH=0 cc -std=c11 -O3 -Wall -Wextra -Werror \
      -DNAPI_VERSION=9 -DNODE_GYP_MODULE_NAME=ghostty_vt_proof -shared -fPIC \
      -I"$node_include" -I"$stage/install/include" \
      "$proof_dir/src/addon.c" -L"$stage/install/lib" -lghostty-vt \
      -Wl,-rpath,'$ORIGIN/install/lib' -ldl -lpthread -lm \
      -o "$stage/ghostty_vt_proof.node"
    strip --strip-all "$stage/ghostty_vt_proof.node"
    ;;
  *) echo "unsupported build host: $(uname -s)" >&2; exit 69 ;;
esac

shasum -a 256 "$stage/ghostty_vt_proof.node"
if [[ "$(uname -s)" == "Linux" ]]; then
  shasum -a 256 "$stage/install/lib/libghostty-vt.so.0.1.0"
fi
mv "$stage" "$output_dir"
trap - EXIT
