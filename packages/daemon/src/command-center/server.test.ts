...
describe("remote access auth", () => {
  it("allows local bypass token when no remote token is configured", async () => {
    const app = createApp({
      remoteAccess: { bindHostname: "0.0.0.0", token: null, localBypassToken: "local-token" },
    });

    const accepted = await app.request("/api/auth/token", {
      method: "POST",
  ...
  it("accepts local bypass or remote bearer tokens when remote access is enabled", async () => {
    const app = createApp({
      remoteAccess: {
        bindHostname: "0.0.0.0",
        token: "remote-token",
        localBypassToken: "local-token",
      },
    });

    const missing = await app.request("/api/auth/token", {
      method: "POST",
      body: JSON.stringify({ userId: "test" }),
    });
    expect(missing.status).toBe(401);

    const remote = await app.request("/api/auth/token", {
      method: "POST",
    ...
    const local = await app.request("/api/auth/token", {
      method: "POST",
      headers: { Authorization: "Bearer local-token" },
      body: JSON.stringify({ userId: "test" }),
    });
    ...
    const wrong = await app.request("/api/auth/token", {
      method: "POST",
  ...
  it("accepts query-string remote tokens but does not exempt loopback clients", async () => {
    const app = createApp({
      remoteAccess: { bindHostname: "0.0.0.0", token: "remote-token" },
    });

    const queryToken = await app.request("/api/auth/token?token=remote-token", {
      method: "POST",
      body: JSON.stringify({ userId: "test" }),
    });
    ...
    const noToken = await strictApp.request("/api/auth/token", {
      method: "POST",
      body: JSON.stringify({ userId: "test" }),
    });
