...
class FakeBridge extends EventEmitter {
  ...
  restartWith(cols: number, rows: number, options?: PtySpawnOptions): void {
    this.restartCount += 1;
    // Mirror real PtyBridge.restartWith semantics: stop + reset replay,
    // then spawn with the new options.
    this.replay = Buffer.alloc(0);
    this.spawn(cols, rows, options);
  }
...
describe("handlePtyWebSocket", () => {
  ...
  it("respawns the bridge when reconnect requests a different cwd", async () => {
    const registry = new PtyBridgeRegistry();
    const bridge = new FakeBridge();
    const ws1 = new MockWebSocket();
    handlePtyWebSocket(ws1, "tab-cwd", {
      registry,
      createBridge: () => bridge,
      idleMs: 1000,
    });

    ws1.receive(JSON.stringify({ type: "init", cols: 80, rows: 24, cwd: "/old/dir" }));
    await waitFor(() => bridge.running === true, "first fake PTY spawn");
    expect(bridge.cwd).toBe("/old/dir");
    bridge.emitOutput(Buffer.from("stale-output"));
    ws1.clientClose();

    const ws2 = new MockWebSocket();
    handlePtyWebSocket(ws2, "tab-cwd", {
      registry,
      createBridge: () => {
        throw new Error("should reuse bridge");
      },
    ...
    ws2.receive(JSON.stringify({ type: "init", cols: 100, rows: 30, cwd: "/new/dir" }));

    await waitFor(() => bridge.restartCount === 1, "bridge restart on cwd change");
    expect(bridge.cwd).toBe("/new/dir");
    expect(bridge.cols).toBe(100);
    expect(bridge.rows).toBe(30);

    // Stale replay must NOT be sent — the prior process is gone.
    const binaryFrames = ws2.sent.filter((frame) => Buffer.isBuffer(frame.data));
    expect(binaryFrames).toEqual([]);
    const replayEnd = jsonFrames(ws2).find((frame) => frame.type === "replay-end");
    expect(replayEnd).toEqual({ type: "replay-end", bytes: 0 });

    registry.shutdown();
  });

  it("keeps the resize+replay path when reconnect cwd matches", async () => {
    const registry = new PtyBridgeRegistry();
    const bridge = new FakeBridge();
    const ws1 = new MockWebSocket();
    handlePtyWebSocket(ws1, "tab-same-cwd", {
      registry,
      createBridge: () => bridge,
      idleMs: 1000,
    });

    ws1.receive(JSON.stringify({ type: "init", cols: 80, rows: 24, cwd: "/repos/alpha" }));
    await waitFor(() => bridge.running === true, "first fake PTY spawn");
    bridge.emitOutput(Buffer.from("keep-me"));
    ws1.clientClose();

    const ws2 = new MockWebSocket();
    handlePtyWebSocket(ws2, "tab-same-cwd", {
      registry,
      createBridge: () => {
        throw new Error("should reuse bridge");
      },
    ...
    ws2.receive(JSON.stringify({ type: "init", cols: 100, rows: 30, cwd: "/repos/alpha" }));

    await waitFor(
      () => ws2.sent.some((frame) => Buffer.isBuffer(frame.data)),
      "replay frame on same-cwd reconnect",
    );
    expect(bridge.restartCount).toBe(0);
    expect(ws2.sent[0]?.data).toEqual(Buffer.from("keep-me"));

    registry.shutdown();
  });

  it("surfaces a typed cwd-not-found error frame when spawn fails on missing cwd", async () => {
    class CwdFailingBridge extends FakeBridge {
      override spawn(cols: number, rows: number, options?: PtySpawnOptions): void {
        // Simulate the real PtyBridge raising TerminalCwdError before
        // any process state changes.
        const cwd = options?.cwd ?? "/missing";
        throw new TerminalCwdError({ cwd, reason: "notFound" });
      }
    }

    const ws = new MockWebSocket();
    handlePtyWebSocket(ws, "tab-bad-cwd", {
      createBridge: () => new CwdFailingBridge(),
    });

    ws.receive(JSON.stringify({ type: "init", cols: 80, rows: 24, cwd: "/missing" }));

    await waitFor(
      () => jsonFrames(ws).some((frame) => frame.type === "error"),
      "typed cwd error frame",
    );
    const errorFrame = jsonFrames(ws).find((frame) => frame.type === "error");
    expect(errorFrame?.reason).toBe("cwd-not-found");
    expect(errorFrame?.cwd).toBe("/missing");
    expect(String(errorFrame?.message)).toContain("/missing");
    expect(ws.closeCount).toBe(1);
  });

  it("surfaces cwd-not-directory and cwd-stat-failed reasons distinctly", async () => {
    const cases: Array<{
      reason: "notDirectory" | "statFailed";
      wire: "cwd-not-directory" | "cwd-stat-failed";
    }> = [
      { reason: "notDirectory", wire: "cwd-not-directory" },
      { reason: "statFailed", wire: "cwd-stat-failed" },
    ];

    for (const { reason, wire } of cases) {
      class CwdFailingBridge extends FakeBridge {
        override spawn(cols: number, rows: number, options?: PtySpawnOptions): void {
          const cwd = options?.cwd ?? "/x";
          throw new TerminalCwdError({ cwd, reason });
        }
      }
      const ws = new MockWebSocket();
      handlePtyWebSocket(ws, `tab-${wire}`, {
        createBridge: () => new CwdFailingBridge(),
      });
      ws.receive(JSON.stringify({ type: "init", cols: 80, rows: 24, cwd: "/somewhere" }));
      await waitFor(
        () => jsonFrames(ws).some((frame) => frame.type === "error"),
        `error frame for ${wire}`,
      );
      const errorFrame = jsonFrames(ws).find((frame) => frame.type === "error");
      expect(errorFrame?.reason).toBe(wire);
      expect(errorFrame?.cwd).toBe("/somewhere");
    }
  });
