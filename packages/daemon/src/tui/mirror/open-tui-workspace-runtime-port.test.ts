...
describe("OpenTUI WorkspaceClient runtime port", () => {
  ...
  it.each([undefined, "window.final"])(
    "opens visible panes and retains latest scoped fit %s while a hidden seed arrives",
    async (finalWindow) => {
      vi.useFakeTimers();
      try {
        ...
        const deliverSeed = (index: number) => {
          ...
          for (const chunk of seed.chunks)
            test.options().onTerminalDelivery(paneIds[index]!, chunk);
        };
        setTimeout(() => {
        ...
        for (let index = 0; index < 100; index++)
          expect(port.fitViewport(120 + index, 40, "window.intermediate")).toBe(fit);
        expect(port.fitViewport(219, 40, finalWindow)).toBe(fit);
        await Promise.resolve();
        ...
        else expect(test.client.fitViewport).toHaveBeenCalledExactlyOnceWith(219, 40, finalWindow);
        await expect(port.fitViewport(219, 41, "window.scoped")).resolves.toBe("ok");
        expect(test.client.fitViewport).toHaveBeenLastCalledWith(219, 41, "window.scoped");
        await port.close();
