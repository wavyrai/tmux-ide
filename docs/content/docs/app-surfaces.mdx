...
A pane can opt into a host-native document surface without ceasing to be a real
tmux pane:
...
Markdown renders through OpenTUI in the terminal app and semantic components in
the web/Electron GUI. Raster images and animated GIFs render in the GUI; a
generic terminal gets an explicit fallback until tmux-ide has negotiated a
pixel-image protocol. Cards select trusted, registered host components. Raw HTML
or React source is never executed from pane output—the tmuxy-style boundary is
strict data selecting code that shipped with the host. Press **Ctrl-C** to clear
the document and return to the underlying terminal.
...
as a project. Recently opened folders are one click away, and bare `tmux-ide`
opens this app by default; `app.frontDoor: false` restores the historical
cockpit entry if desired. See
...
`~/.tmux-ide/config.json`. See [workspace configuration](/docs/configuration).
