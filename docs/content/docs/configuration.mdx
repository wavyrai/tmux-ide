...
- **`app.frontDoor`** controls the default entry point. Bare `tmux-ide` launches
  the unified visual app by default (`true`); set it to `false` to restore the
  historical home cockpit. Project config auto-launch and explicit
  `tmux-ide team` are unaffected.
