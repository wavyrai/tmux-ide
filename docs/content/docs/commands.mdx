...
protocol exits non-zero so the host can ask the user to restart or upgrade. A
dead PID or failed health probe is stale state: it is removed and replaced by
the new owner.
