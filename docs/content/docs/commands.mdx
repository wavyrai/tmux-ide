...
| `tmux-ide switcher` | Compact session picker (the `⌥p` popup) |
| `tmux-ide cheatsheet` | Print the key cheat sheet (`⌥k`) |
| `tmux-ide menu [--client N]` | Open the actions menu (`⌥m` / right-click) |
| `tmux-ide popup <widget>` | Open a widget panel (`explorer` / `changes` / `config`) |
| `tmux-ide sidebar-toggle [--session S]` | Toggle the nav column (`⌥b`) |
...
Once a session is adopted, the whole UI is a keystroke away:

| Key | Opens |
| --- | --- |
| `⌥h` | Home cockpit from anywhere |
| `⌥p` | Switch session |
| `⌥k` | Cheat sheet (all keys) |
| `⌥m` | Actions menu (or right-click any pane / the bar) |
| `⌥e` `⌥g` `⌥,` | File / changes / config panels |
| `⌥b` | Sidebar |

Rebind any of them in `~/.tmux-ide/config.json` — see
