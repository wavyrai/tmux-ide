---
title: CLI reference
description: The supported tmux-ide commands and flags in 2.9.0-beta.24
---

This page follows the public surface printed by `tmux-ide --help` in
`2.9.0-beta.24`. `--json` is available on the scripting commands that explicitly
...
## Multiple tmux servers (development)

The development build can register multiple tmux servers on one machine. List
registrations to get the opaque server ID, then use it to select a server:

```bash
tmux-ide servers list --json
tmux-ide servers add --socket-name work --name Work --json
tmux-ide servers add --socket-path /absolute/path/to/socket --name Custom --json
tmux-ide servers sessions <server-id> --json
tmux-ide servers create <server-id> --session-name Work --dir /project/path --json
tmux-ide app --server <server-id> [session]
tmux-ide servers remove <server-id> --json
```

Add `--ssh HOST` to manage a remote machine's registrations or open its app.
Socket names and paths are resolved on that remote machine. Registration does
not create a native tmux server, and removing a registration does not kill its
sessions. An offline server remains visible until removed.

The app groups sessions by machine and server. A single server uses a compact
view. Saved tabs retain the exact server and session incarnation; a replacement
session requires selecting it again. Explicit `--server` opens in foreground
mode and cannot be combined with hosted or headless mode. An older daemon
without the server API reports an unsupported capability instead of selecting
its default server.
...
| `tmux-ide daemon info [--json]`                          | Identity, supervisor and actual log destination of the daemon; labels stale logs  |
...
## Identity and log provenance

`tmux-ide daemon info --json` reports the current daemon instance id, version,
pid and liveness, how it was launched (manual, launchd, systemd or embedded in a
host) and where its stdout/stderr actually go. The destination is stamped into
the daemon record by the running process at startup, so the report does not
guess from file names. Any other log-like file discovered under the state home
is listed with its last write time and the pid it mentions, labeled
`historical` or `unattributed`; only the live destination is `current`. The
report never contains the auth token. `tmux-ide doctor` shows the same summary
as one row.
