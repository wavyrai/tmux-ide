...
is about the vocabulary not being _spoken_ — tokens exist and go unused,
...
| R4.7 | Tooltip draws its silhouette with a border _and_ a shadow — the ring should draw it alone     | `ui-system.css:388` |
...
`after/` screenshots sit beside `before/`, same nine surfaces, same two
appearances, same 3x density.

| Commit     | Concern                                            | Closes                                                                             |
| ---------- | -------------------------------------------------- | ---------------------------------------------------------------------------------- |
| `cda4a02d` | Icon library behind the semantic vocabulary        | R1.1–R1.6                                                                          |
| `72b85c1c` | Control recipes, one focus ring, system typography | R2.1–R2.7, R2.13, R3.1, R3.6–R3.8, R4.6–R4.8, R5.10, R5.11, R6.2, R7.1, R7.3, R7.6 |
| `635c4f07` | Border and shadow purge in the shell               | R3.2–R3.5, R4.1–R4.5, R5.1–R5.9                                                    |
| `29886f55` | The named type ramp, everywhere                    | R6.1, R6.3–R6.7                                                                    |
| `2e843926` | Icons on bare-text affordances                     | R1.7, R1.9, R1.12                                                                  |
| `f7cfd59b` | Five chrome control families into one recipe       | R2.8–R2.12, R2.14, R7.2, R7.4, R7.5, R7.7                                          |
| `1f421a42` | State in shape; last hand-drawn SVG retired        | R1.4, R1.13, R1.15                                                                 |
| `b15aaa10` | Runtime state and onboarding facts get glyphs      | R1.10, R1.11                                                                       |

### Closing counts

| Rule                | Found | Fixed | Exception | Withdrawn |
| ------------------- | ----- | ----- | --------- | --------- |
| R1 Icons            | 16    | 14    | 1 (R1.8)  | 1 (R1.14) |
| R2 Control recipes  | 14    | 14    | —         | —         |
| R3 Focus            | 8     | 8     | —         | —         |
| R4 Borders          | 8     | 8     | —         | —         |
| R5 Shadows          | 11    | 11    | —         | —         |
| R6 Typography       | 7     | 7     | —         | —         |
| R7 Radius + spacing | 7     | 7     | —         | —         |
| **Total**           | 71    | 69    | 1         | 1         |
...
**R1.14 — "Command palette rows have no icons."** They do.
`command-palette.tsx` renders a semantic icon per row inside
`.command-palette__icon`, grouped exactly as the contract describes. The item
was written from the rule rather than from the code and should not have been
raised. No change was made.

---

## Exceptions: violations that are correct here

**R1.8 — fleet and session rows carry a status dot, not a glyph.**

The contract asks for an icon on every affordance, and these rows have one
device: a small colored dot reporting agent state. Adding a workspace glyph
beside it would put two marks in front of every row in a list whose whole job
is to be scanned quickly, and the second mark would be the same folder icon on
every row — decoration that adds a column and distinguishes nothing.

The dot is also load-bearing in a way an icon is not: it is ground-truth agent
state, and the row's name says what the row is. This is the one place where
the honest answer is that the status device is the identity. The status strip
did get a glyph in `1f421a42`, because there the state is the _only_ content —
different problem, different answer.

Worth revisiting if the fleet ever carries user-chosen workspace emoji, at
which point `WorkspaceIdentity` is already built and the row would be showing
something the user chose rather than a generic folder.

**Border over 1px: the scrollbar thumb.** `styles.css` keeps a
`border: 2px solid transparent` on `::-webkit-scrollbar-thumb`. It paints
nothing — it is the standard `background-clip: content-box` inset that insets
the thumb inside its track. Not an edge, so not a border in the sense rule 4
governs.
...
**One icon package, not two.** The contract names `@hugeicons/react` and
`@hugeicons/core-free-icons`. This renderer is Solid, not React; adding the
React wrapper would pull React into a Solid bundle to render SVG that
`core-free-icons` already ships as plain data. Only `core-free-icons` was
added, and the drawing component is the Solid `Icon` in ui-system. That is the
same architecture the contract describes — one wrapper, uniform and swappable —
reached without a second framework.

**caption-3 (10px) extends the ramp.** Rule 6 says named ramp only, and the
ramp's floor was 11px against 43 declarations at 9px and 10px. Rounding all of
them to 11px would have changed dense chrome the contract did not ask to
change; leaving them numeric would have left rule 6 unmet. One named micro
step, with 9px rounding up to it, resolves both. Flagged because it adds a step
the source language does not have.

**Display steps are named rather than removed.** The ramp comment always
allowed launch and onboarding type to be larger by intent; it just had no
names for those sizes, so they lived as raw clamps. They are now
`--sf-text-display-{1,2,3}` and `--sf-text-title`.
