...
describe("AppWindow command intents", () => {
  ...
  it("maximizes through deterministic focus, move, resize commands and remembers restore geometry", () => {
    ...
    const maximized = toggleAppWindowMaximizeIntent({
      ...
      state: { mode: "restored", restoreRect: null },
      source: "mouse",
    });

    expect(maximized.state).toEqual({ mode: "maximized", restoreRect: currentRect });
    expect(maximized.commands.map(({ command }) => command.type)).toEqual([
      "window.focus",
      "window.move",
      "window.resize",
    ]);
    ...
    expect(restored.state).toEqual({ mode: "restored", restoreRect: null });
  });
});
